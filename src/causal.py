...
def build_paired_dataset(
    ...
    for date_str, pm_values in pm25_daily.items():
        ...
        if hotspots:
            x_val = max(
                h["brightness"]
                / (1.0 + haversine_km(city_lat, city_lon, h["lat"], h["lon"]))
                for h in hotspots
            )
        else:
            x_val = 0.0
...
async def get_causal_attribution(city: str) -> dict:
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        ...
        if sensor_id is None:
            return _fallback(
                display_name, anomaly_result, "No PM2.5 sensor found near this city."
            )
    ...
    if ate is None:
        return _fallback(
            display_name,
            anomaly_result,
            "DoWhy estimation failed on the available data.",
        )
